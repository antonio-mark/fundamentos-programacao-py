...
def calcula_escolha_tres(tres_cidades):
    ...
    valor_distancia_primeira_segunda = int(matriz[posicao_primeira_cidade + 1][posicao_segunda_cidade])
    valor_distancia_primeira_terceira = int(matriz[posicao_primeira_cidade + 1][posicao_terceira_cidade])
    valor_distancia_segunda_terceira = int(matriz[posicao_segunda_cidade + 1][posicao_terceira_cidade])
    valor_distancia_terceira_segunda = int(matriz[posicao_terceira_cidade + 1][posicao_segunda_cidade])
...
def calcula_escolha_quatro(array_cidades, custo_por_km):
    distancia_total = 0

    for i, cidade in enumerate(array_cidades):
        if i == len(array_cidades) -1:
            break
        posicao_primeira_cidade = matriz[0].index(array_cidades[i])
        posicao_proxima_cidade = matriz[0].index(array_cidades[i+1])

        distancia_de_para = matriz[posicao_primeira_cidade + 1][posicao_proxima_cidade]
    
        distancia_total += int(distancia_de_para) 
        print('\n')
        print(f"De {array_cidades[i]} para {array_cidades[i+1]} distância {distancia_de_para}")
    

    custo_total = int(distancia_total) * float(custo_por_km)
    consumido = int(distancia_total) * 2.57
    dias = round(int(distancia_total)/583)

    print(f'\nDistância total percorrida: {distancia_total}Km')
    print('\nCusto total: R${}'.format(custo_total))
    print('\nLitros de combustível consumidos: {:.2f}L'.format(consumido))
    print('\nDias em média para finalizar a viagem {} dias'.format(dias))
...
def escolha_tres(tres_cidades):
    cidades = input("Informe as três cidades separadas por virgula: ").upper()
...
def escolha_quatro(custo_por_km):
    cidade = ''
    while cidade.upper() != 'FIM' or not cidades_escolha_quatro:
        cidade = ''
        ...
        if cidade == 'FIM':
            if len(cidades_escolha_quatro) < 3:
                print('\nQuantidade menor que 3, comando recusado')
                cidades_escolha_quatro.clear()
                continue

            for i in cidades_escolha_quatro:
                if i not in matriz[0]:
                    print('\nUma cidade informada é inválida')
                    cidades_escolha_quatro.clear()
                    continue       
    calcula_escolha_quatro(cidades_escolha_quatro, custo_por_km)
